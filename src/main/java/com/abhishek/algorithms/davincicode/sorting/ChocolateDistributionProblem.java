...
public class ChocolateDistributionProblem {
	...
	public static void main(String[] args) {
		...
		int m = 3;
		int n = arr.length;
		System.out.println("Minimum difference using brute force = : "
				+ chocolateDistributionNaive(arr, n, m));
	...
	public static int chocolateDistributionNaive(int[] arr, int n, int m) {
		List<Integer> result = chocolateDistributionProblem_1(arr,
				new LinkedList<>(), n - 1, m);
		return ((result.size() == m)
				? result.get(m - 1) - result.get(0)
				: (-1));
	}

	/**
	 * Logic: Brute force approach The idea is to generate all the possible
	 * subsets and find the minimum difference between the last element and the
	 * first element of an array.
	 * 
	 * @param arr:
	 *            Input array
	 * @param chocolates:
	 *            The container to store the subsets
	 * @param n:
	 *            input size of an array
	 * @param m:
	 *            input size of the container
	 * @return
	 */
	public static List<Integer> chocolateDistributionProblem_1(int[] arr,
			List<Integer> chocolates, int n, int m) {
		if (n < 0 || chocolates.size() == m)
			return chocolates;

		List<Integer> temp = new LinkedList<Integer>(chocolates);
		temp.add(arr[n]);
		List<Integer> include = chocolateDistributionProblem_1(arr, temp, n - 1,
				m);
		List<Integer> exclude = chocolateDistributionProblem_1(arr, chocolates,
				n - 1, m);
